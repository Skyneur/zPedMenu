...
const StarIcon = ({ filled = false, className = "" }: { filled?: boolean; className?: string }) => (
  filled ? (
    ...
    <svg 
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M8.59215 3.50773L7.40706 0.954351C7.04875 0.182341 5.95124 0.182341 5.59294 0.954352L4.40785 3.50773C4.26535 3.81476 3.9772 4.02886 3.64211 4.0767L0.957183 4.46002C0.145632 4.57588 -0.1894 5.56536 0.384819 6.15044L2.40748 8.21134C2.63306 8.44118 2.73414 8.7656 2.67904 9.0829L2.18373 11.935C2.0398 12.7638 2.92109 13.3891 3.65583 12.9796L6.01315 11.6657C6.3158 11.497 6.6842 11.497 6.98685 11.6657L9.34925 12.9824C10.0831 13.3915 10.9637 12.7681 10.8217 11.94L10.3206 9.01709C10.2658 8.69721 10.3696 8.3706 10.5991 8.14108L12.586 6.15377C13.1686 5.57103 12.8359 4.57324 12.0201 4.45678L9.35789 4.0767C9.0228 4.02886 8.73465 3.81476 8.59215 3.50773Z" fill="#070707"/>
    </svg>
  )
);
...
const categories = ['All', 'Civilian', 'Characters', 'Animals', 'Police', 'Gang'];
...
const getPedCategory = (model: string): string => {
  ...
  if (model.startsWith('cs_') || model.startsWith('csb_') || model.startsWith('ig_')) return 'Characters';
  if (model.startsWith('g_')) return 'Gang';
  if (model.includes('cop') || model.includes('police') || model.includes('sheriff') || model.includes('swat')) return 'Police';
  return 'Civilian';
};
...
const enrichedPeds: PedData[] = pedsData
  ...
  .filter((ped, index, self) => 
    index === self.findIndex(p => p.model === ped.model)
  );

const PedMenu: React.FC = () => {
  console.log('PedMenu component is loading...'); // Debug log
  ...
  console.log('PedMenu state:', { isVisible, activeTab }); // Debug log

  // Référence pour la grille pour pouvoir contrôler le scroll
  const gridRef = useRef<HTMLDivElement>(null);

  // Écouter les messages NUI pour afficher/masquer le menu
  useNuiEvent('show_menu', () => {
    console.log('NUI: Received show_menu event');
    setIsVisible(true);
  });

  useNuiEvent('hide_menu', () => {
    console.log('NUI: Received hide_menu event');
    setIsVisible(false);
  });
  ...
  const getFilteredPeds = (): PedData[] => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(ped =>
        ped.model.toLowerCase().includes(searchTerm.toLowerCase()) ||
        formatPedName(ped.model).toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  // Ne pas rendre le composant si pas visible
  if (!isVisible) {
    console.log('Menu not visible, returning null');
    return null;
  }

  console.log('Rendering PedMenu...'); // Debug log

  return (
    <>
      {/* Image de fond pour tout le site */}
      <div 
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-zinc-400">
              {filteredPeds.length} peds {showFavorites && favorites.length > 0 && `(${favorites.length} favoris)`}
            </span>
            ...
            <button
              ...
              title={showFavorites ? 'Voir tous les peds' : 'Voir les favoris'}
            >
              {/* Ligne rouge en bas au hover - seulement si pas actif */}
              {!showFavorites && (
        ...
        <div className="flex justify-between mb-4 relative">
          ...
          {/* Indicateur animé de l'onglet actif - seulement visible quand on n'est pas en mode favoris */}
          {!showFavorites && (
            <motion.div
        ...
        {/* Ligne de séparation fine rouge */}
        <div className="w-full h-0.5 bg-[#FF3837] mb-4 opacity-50"></div>

        {/* Grille avec transition entre les vues */}
            ...
            >
              {filteredPeds.map((ped) => (
                <div key={ped.model} className="relative">
                  ...
                  >
                    {/* Contour animé sélectionné avec SVG pour suivre parfaitement les coins arrondis */}
                    {selectedPed === ped.model && (
                      ...
                      <div className="w-16 h-16 bg-zinc-700 rounded-lg overflow-hidden transition-all flex items-center justify-center relative mt-1">
                        <img
                          ...
                          onError={(e) => {
                            ...
                            target.src = `https://via.placeholder.com/80x100/374151/ffffff?text=${ped.model.slice(0, 3)}`;
                          }}
                        />
                      </div>
                      <span 
                  ...
                  <button
                    ...
                    title={favorites.includes(ped.model) ? 'Retirer des favoris' : 'Ajouter aux favoris'}
                  >
                    <StarIcon 
        ...
        {filteredPeds.length === 0 && (
          <div className="text-center py-8 text-zinc-400">
            {showFavorites ? (
              <>
                <p className="text-lg font-medium">Aucun favori sauvegardé</p>
                <p className="text-sm mt-2 text-zinc-500">
                  Cliquez sur <span className="inline-flex items-center justify-center w-6 h-6 bg-white/10 rounded-full mx-1">
                    <StarIcon className="w-3 h-3" />
                  </span> pour ajouter des peds en favoris
                </p>
              </>
            ) : (
              <>
                <p className="text-lg font-medium">Aucun ped trouvé</p>
                <p className="text-sm mt-2 text-zinc-500">Essayez un autre terme de recherche</p>
              </>
            )}
          </div>
        )}
